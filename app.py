...
# Google Drive File ID — set as environment variable on Railway
...
CREATE INDEX IF NOT EXISTS idx_co     ON contacts(company);
...
def download_from_gdrive(file_id, dest_path):
    ...
    print(f"[DOWNLOAD] Fetching Excel (id={file_id})...")
    ...
    for url in urls:
        try:
            ...
            if size < 5000:
                print(f"\n  Too small ({size} bytes), trying next...")
                os.remove(dest_path)
                continue

            print(f"\n[DOWNLOAD] Done — {size//1024//1024} MB")
            return True
...
def load_excel():
    ...
    if not os.path.exists(DATA):
        if GDRIVE_FILE_ID:
            ok = download_from_gdrive(GDRIVE_FILE_ID, DATA)
            if not ok:
                print("[WARN] Could not download Excel.")
                return
        else:
            print("[WARN] No Excel file and no GDRIVE_FILE_ID set.")
            return
    ...
    for _, r in df.iterrows():
        rows.append(("teacher",
            clean(r.get("FULL NAME / ENTITY")), clean(r.get("MOBILE (PRIMARY)")),
    ...
    for _, r in df.iterrows():
        ...
        try:
            ...
            mob = clean(mob)
        rows.append(("gst",
            clean(r.get("FULL NAME / ENTITY")), mob,
    ...
    with get_db() as conn:
        for i in range(0, len(rows), 5000):
            ...
            conn.commit()
            print(f"  Saved {min(i+5000,len(rows)):>7,}/{len(rows):,}", end="\r")
    print(f"\n[LOAD] Complete — {len(rows):,} records.")
...
def add_contact():
    ...
    execute("INSERT INTO activities (contact_id,type,note) VALUES (?,?,?)",
            (cid, "created", "Contact created"))
    return jsonify({"id": cid, "message": "Contact created"}), 201

@app.route("/api/contacts/<int:cid>", methods=["PUT"])
def update_contact(cid):
    d = request.json or {}
    execute("""UPDATE contacts SET stage=?,score=?,notes=?,company=?,
               income=?,district=?,name=?,mobile=? WHERE id=?""",
            (d.get("stage"), d.get("score"), d.get("notes"), d.get("company"),
             d.get("income"), d.get("district"), d.get("name"), d.get("mobile"), cid))
    execute("INSERT INTO activities (contact_id,type,note) VALUES (?,?,?)",
            (cid, "updated", d.get("notes","Updated")))
...
def add_pipeline():
    d = request.json or {}
    pid = execute("""INSERT INTO pipeline (contact_id,name,segment,stage,value,notes)
                     VALUES (?,?,?,?,?,?)""",
...
def analytics():
    ...
    inc_dist = query("""SELECT CASE
        WHEN income < 500000  THEN '< 5L'
        WHEN income < 1500000 THEN '5L - 15L'
        WHEN income < 3000000 THEN '15L - 30L'
        ELSE '30L+' END as bracket, COUNT(*) cnt
        FROM contacts WHERE income IS NOT NULL GROUP BY bracket""")
    ...
    return jsonify({
        ...
        "income_distribution": [dict(r) for r in inc_dist],
        "gst_commissionerate": [dict(r) for r in gst_comm],
...
def search():
    ...
    s = f"%{q}%"
    rows = query("""SELECT id,segment,name,mobile,company,district,score
                    FROM contacts WHERE name LIKE ? OR mobile LIKE ? OR company LIKE ? OR gstin LIKE ?
                    ORDER BY score DESC LIMIT 20""", (s,s,s,s))
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 PropTrack CRM running at http://localhost:{port}\n")
